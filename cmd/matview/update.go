...
func runUpdate(cmd *cobra.Command, args []string) error {
	...
	// --no-schedule is a directive to remove the schedule; only a true value is a real change
	// (an explicit --no-schedule=false is the default and means nothing).
	...
	if !cmd.Flags().Changed("private") && !cmd.Flags().Changed("performance") &&
		!cmd.Flags().Changed("cron") && !noSchedule &&
