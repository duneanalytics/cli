...
// A no-flag update must error before any API call — updating re-executes the query and spends
// credits, so a no-op must not silently trigger a refresh.
func TestUpdateNoFlagsErrors(t *testing.T) {
	mock := &mockClient{
		getFn: func(string) (*models.GetMaterializedViewResponse, error) {
			t.Fatal("GetMaterializedView must not be called for a no-op update")
			return nil, nil
		},
		upsertFn: func(models.UpsertMaterializedViewRequest) (*models.UpsertMaterializedViewResponse, error) {
			t.Fatal("UpsertMaterializedView must not be called for a no-op update")
			return nil, nil
		},
	}
	root, _ := newTestRoot(mock)
	root.SetArgs([]string{"matview", "update", "dune.t.result_x"})
	err := root.Execute()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "at least one flag must be provided")
}

// `--no-schedule=false` is the default and not a real change, so it must be rejected like a
// no-flag update (it would otherwise pass the guard and trigger a no-op re-execution).
func TestUpdateNoScheduleFalseErrors(t *testing.T) {
	mock := &mockClient{
		getFn: func(string) (*models.GetMaterializedViewResponse, error) {
			t.Fatal("GetMaterializedView must not be called for --no-schedule=false with no other change")
			return nil, nil
		},
		upsertFn: func(models.UpsertMaterializedViewRequest) (*models.UpsertMaterializedViewResponse, error) {
			t.Fatal("UpsertMaterializedView must not be called for a no-op update")
	...
	root, _ := newTestRoot(mock)
	root.SetArgs([]string{"matview", "update", "dune.t.result_x", "--no-schedule=false"})
	err := root.Execute()
	require.Error(t, err)
	assert.Contains(t, err.Error(), "at least one flag must be provided")
}
